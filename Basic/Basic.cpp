...
/*低优先级守护线程*/
	static QueueHandle_t LPDaemon_Queue;
	struct LPDaemonTaskInf
	{
	...
	{
		LPDaemonTaskInf task_inf;
		task_inf.task = task;
		task_inf.pvParameter1 = pvParameter1;
		task_inf.ulParameter2 = ulParameter2;
		xQueueSendFromISR( LPDaemon_Queue, &task_inf, xHigherPriorityTaskWoken );
	}
	void xLPDaemonTaskCall( void (*task)( void *pvParameter1, uint32_t ulParameter2 ),
																	void* pvParameter1,
																	uint32_t ulParameter2,
																	TickType_t xTicksToWait )
	{
		...
		task_inf.pvParameter1 = pvParameter1;
		task_inf.ulParameter2 = ulParameter2;
		xQueueSend( LPDaemon_Queue, &task_inf, xTicksToWait );
	}
	
	static void LPDaemonTask(void* pvParameters)
	{
		while(1)
		{		
			LPDaemonTaskInf task_inf;
			xQueueReceive( LPDaemon_Queue, &task_inf, portMAX_DELAY );
...
{	
  ...
  /* Enable D-Cache---------------------------------------------------------*/
	...
	LPDaemon_Queue = xQueueCreate( 20, sizeof(LPDaemonTaskInf) );
	xTaskCreate( LPDaemonTask, "LPDaemonTask", 1024, NULL, SysPriority_UserTask, NULL);
}
