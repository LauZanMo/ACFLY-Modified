...
/*AK8975校准数据*/
	static vector3<double> AK8975_ASA;
	...
	static bool AK8975_present = false;
	
	static bool AK8975_init()
	{
		/*AK8975初始化*/
			//检验Chip ID
			TxBuf[0] = (1<<7) | 0;
			...
			/*读AK8975校准数据*/
				...
				PullUp_AK8975_CS;
				AK8975_ASA.x = (RxBuf[1] - 128)*0.5/128 + 1;
				AK8975_ASA.y = (RxBuf[2] - 128)*0.5/128 + 1;
				AK8975_ASA.z = (RxBuf[3] - 128)*0.5/128 + 1;
				os_delay(0.01);
	...
	*/
	...
	/*BMI088初始化*/
		/*加速度计初始化*/
			...
			set_IMU_Accelerometer_UpdateFreq( 0 , 1000 );
...
extern "C" void TIM16_IRQHandler()
{
	...
	//判断是否读BMI088 Acc
	static uint8_t BMI088Acc_counter = 0;
	if( ++BMI088Acc_counter >= 2 )
	{
		BMI088Acc_counter = 0;
...
static void SPL06TCB( void *pvParameter1, uint32_t ulParameter2 )
{
	...
	pressure_internal_temperature = IMU_Temperature[1] = temperature;
	...
	if( pressure > 0 )
	{
		...
		PositionSensorUpdatePosition( internal_baro_sensor_index , position , true , -1, -1, trust );
	}
}
