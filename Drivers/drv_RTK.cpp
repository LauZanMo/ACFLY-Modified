...
GPS_Present:
	...
	if( ReadParamGroup( "GPS2Cfg", (uint64_t*)&gps_cfg, 0 ) == PR_OK )
	{
		...
		//注册传感器
		PositionSensorRegister( default_rtk_sensor_index , \
	...
	while(1)
	{
		...
		if( driver_info.port.read( &r_data, 1, 2, 0.1 ) )
		{
          ...
          /*更新RTC时时间(本地时间)*/							
					...
					if( z_available )
						PositionSensorChangeDataType( default_rtk_sensor_index, Position_Sensor_DataType_sv_xyz );
					else
						PositionSensorChangeDataType( default_rtk_sensor_index, Position_Sensor_DataType_sv_xy );
					
					//信任度
					double xy_trustD = pack->hAcc * 0.1;
					double z_trustD = pack->vAcc * 0.1;
					PositionSensorUpdatePositionGlobalVel( default_rtk_sensor_index, position_Global, velocity, gps_available, -1, xy_trustD, z_trustD, addition_inf );
				}
          ...
          if( (pack->valid & (uint8_t)0x03) == (uint8_t)0x03 )
			...
			{	//接收不到数据
				PositionSensorUnRegister( default_rtk_sensor_index );
				//关闭Rtk注入
...
static bool RTK_DriverInit( Port port, uint32_t param )
{
	...
	xTaskCreate( RTK_Server, "RTK", 1024, driver_info, SysPriority_ExtSensor, NULL);
	return true;
}
...
void init_drv_RTK()
{
	...
	ParamGroupRegister( "GPS2Cfg", 2, sizeof(initial_cfg)/8, param_types, param_names, (uint64_t*)&initial_cfg );
	
	PortFunc_Register( 13, RTK_DriverInit );
}