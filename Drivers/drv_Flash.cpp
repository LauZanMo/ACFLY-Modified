...
/*Flash操作*/
	...
	extern "C" bool FlashEraseSectors( uint32_t sector_addr , uint16_t sectors , double waitT )
	{			
	...
	extern "C" bool FlashProgramPages( const uint8_t* buf , uint32_t sector_addr , uint16_t page_addr , uint16_t pages , double waitT )
	{			
		uint32_t addr = sector_addr * PagesInSector*PageSize + page_addr*PageSize;
		EventBits_t result;
		...
		for( uint16_t page = 0 ; page < pages ; ++page )
		{
			...
			QUADSPI->CCR = FunctionMode_IndirectWrite | \
				...
				AddressMode_1Line | AddressSize24 | \
				DataMode_4Line;
			QUADSPI->AR = addr + page*PageSize;
			
			//MDMA发送数据
			MDMA_Channel0->CIFCR = (1<<4) | (1<<3) | (1<<2) | (1<<1) | (1<<0);
			MDMA_Channel0->CTBR = (0<<17) | (0<<16) | (22<<0);
			MDMA_Channel0->CSAR = (uint32_t)(buf + page*PageSize);
	...
	extern "C" bool FlashReadPages( uint8_t* buf , uint32_t sector_addr , uint32_t page_addr , uint16_t pages , double waitT )
	{
		return FlashCMDRead_QuadOutput( buf , pages*PageSize , 0x6b , sector_addr * PagesInSector*PageSize + page_addr*PageSize , 8 , waitT );
	}
	...
	extern "C" uint32_t getFlashPageSize()
	{
		return PageSize;
	}
/*Flash操作*/
