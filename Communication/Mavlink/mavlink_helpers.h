 ...
 */
MAVLINK_HELPER uint16_t mavlink_msg_to_send_buffer(
							...
							double SyncTIMEOUT, double TIMEOUT)
{
	...
	if (msg->magic == MAVLINK_STX_MAVLINK1) {
		...
		buf[4] = msg->compid;
		buf[5] = msg->msgid & 0xFF;
		write_port( &buf[0] , 6, SyncTIMEOUT, TIMEOUT );
		write_port( (uint8_t*)_MAV_PAYLOAD(msg) , msg->len, SyncTIMEOUT, TIMEOUT );
	} else {
		...
		buf[8] = (msg->msgid >> 8) & 0xFF;
		buf[9] = (msg->msgid >> 16) & 0xFF;
		write_port( &buf[0] , 10, SyncTIMEOUT, TIMEOUT );
		write_port( (uint8_t*)_MAV_PAYLOAD(msg), length, SyncTIMEOUT, TIMEOUT );
		signature_len = (msg->incompat_flags & MAVLINK_IFLAG_SIGNED)?MAVLINK_SIGNATURE_BLOCK_LEN:0;
	}
	buf[0] = (uint8_t)(msg->checksum & 0xFF);
	buf[1] = (uint8_t)(msg->checksum >> 8);
	if (signature_len > 0) {
